...
public class VehicleCard {
    public static CardView card(final Fragment frag, final Vehicle vh, final Handler handle, final GoogleApiClient gac){
        ...
        nameText.setText(vh.name);

        TextView modelText = new TextView(frag.getContext());
        LinearLayout.LayoutParams modelTextLp =
                new LinearLayout.LayoutParams(LinearLayout.LayoutParams.MATCH_PARENT,LinearLayout.LayoutParams.WRAP_CONTENT);
        modelText.setLayoutParams(modelTextLp);
        modelText.setText(vh.model);
        topLay.addView(nameText);
        topLay.addView(modelText);
