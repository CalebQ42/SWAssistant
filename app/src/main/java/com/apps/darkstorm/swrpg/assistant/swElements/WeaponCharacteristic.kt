...
data class WeaponCharacteristic(var name: String = "",var value: Int = 0,var advantage: Int = 0): JsonSavable(){
    override fun save(jw: JsonWriter) {
        jw.beginObject()
        jw.name("name").value(name)
        jw.name("value").value(value)
        jw.name("advantage").value(advantage)
        jw.endObject()
    }
    override fun load(jr: JsonReader) {
        ...
        while(jr.hasNext() && jr.peek() != JsonToken.END_OBJECT){
            ...
            when(jName){
                ...
                "advantage"-> advantage = jr.nextInt()
            }
        }
        jr.endObject()
    }
