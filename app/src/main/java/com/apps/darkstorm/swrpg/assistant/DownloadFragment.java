...
import com.apps.darkstorm.swrpg.assistant.ui.cards.StringCard;
...
public class DownloadFragment extends Fragment {
    ...
    public void onViewCreated(View top, @Nullable final Bundle savedInstanceState) {
        ...
        vehic = getActivity().getLayoutInflater().inflate(R.layout.general_list,pager,false);
        mini = getActivity().getLayoutInflater().inflate(R.layout.general_list,pager,false);
        chara = getActivity().getLayoutInflater().inflate(R.layout.general_list,pager,false);
                    ...
                    protected Void doInBackground(Void... params) {
                        ...
                        stor.child(name + ".char").getFile(new File(tmp.getFileLocation(getActivity()))).addOnSuccessListener(new OnSuccessListener<FileDownloadTask.TaskSnapshot>() {
                            ...
                            public void onSuccess(FileDownloadTask.TaskSnapshot taskSnapshot) {
                                AsyncTask<Void,Void,Void> asyncSave = new AsyncTask<Void, Void, Void>() {
                                    @Override
                                    protected Void doInBackground(Void... params) {
                                        if(((SWrpg)getActivity().getApplication()).prefs.getBoolean(getString(R.string.google_drive_key),false)) {
                                ...
                                asyncSave.executeOnExecutor(AsyncTask.THREAD_POOL_EXECUTOR);
                            }
