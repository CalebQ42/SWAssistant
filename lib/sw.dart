...
class SW {
  ...
  bool remove(Editable editable, BuildContext context) {
    if (editable.route != null &&
        observatory.containsRoute(route: editable.route) != null) {
  ...
  bool removeCharacter({String? uid, Character? character}) {
    ...
    if (success &&
        character != null &&
        characters(category: character.category).isEmpty) {
      updateCharacterCategories();
    }
    return success;
  }
  ...
  bool removeMinion({String? uid, Minion? minion}) {
    ...
    if (success &&
        minion != null &&
        minions(category: minion.category).isEmpty) {
      updateMinionCategories();
    }
    return success;
  }
  ...
  bool removeVehicle({String? uid, Vehicle? vehicle}) {
    ...
    if (success &&
        vehicle != null &&
        vehicles(category: vehicle.category).isEmpty) {
      updateVehicleCategories();
    }
    return success;
  }
  ...
  Future<bool> initialSync(BuildContext context) async {
    ...
    for (var fil in await driver!.listFiles("") ?? <drive.File>[]) {
      ...
      if (fil.name!.endsWith(".swcharacter") ||
          fil.name!.endsWith(".swminion") ||
          fil.name!.endsWith(".swvehicle")) {
        //TODO
      }
    }
    return false;
  ...
  Future<bool> driveInit() async {
    ...
    if (foldId == null) {
      foldId = await driver!.createFolderFromRoot("SWChars",
  ...
  static Future<SW> baseInit() async {
    ...
    prefs.setInt(preferences.startCount,
        app.getPreference(preferences.startCount, 0) + 1);
    return app;
  }
  ...
  void manualImport(BuildContext context) {
    ...
    FilePicker.platform.pickFiles(allowMultiple: true).then((value) {
      ...
      if (value == null || value.files.isEmpty) {
        ...
        message.showSnackBar(SnackBar(content: Text(locs.importNone)));
      } else {
        message.clearSnackBars();
        message.showSnackBar(
            SnackBar(content: Text(locs.importSuccess(value.files.length))));
      }
    });
  }
  ...
  static Future<void> testing(String saveDir) async {
    var testFiles = [
      "Big Game Hunter [Nemesis][Testing].swcharacter",
      "Incom T-47 Airspeeder [Testing].swvehicle",
      "Pirate Crew [Testing].swminion"
    ];
...
class SWWidget extends InheritedWidget {
  ...
  const SWWidget({Key? key, required Widget child, required this.app})
