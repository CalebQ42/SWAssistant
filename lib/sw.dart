...
class SW with TopResources {
  ...
  Future<void> postInit(
      LoadingScreenState loadingState, ScaffoldMessengerState messager) async {
    ...
    if (prefs.googleDrive) {
      loadingState.loadingText =
          AppLocalizations.of(loadingState.context)!.driveSyncing;
      var syncd = await syncRemote(onFull: () {
        if (showFullError) {
          messager.showSnackBar(
            SnackBar(
              content:
                  Text(AppLocalizations.of(loadingState.context)!.driveFull),
            ),
          );
          showFullError = false;
          Future.delayed(
              const Duration(minutes: 5), () => showFullError = true);
        }
      });
      if (!syncd) {
        driver = null;
  ...
  Future<bool> syncRemote(
      ...
      NavigatorState? nav,
      VoidCallback? onFull}) async {
    var driveSuccess = false;
    if (prefs.driveFirstLoad) {
      driveSuccess = await _googleDriveSync(true,
          onFull: onFull, context: context, nav: nav);
    } else if (!prefs.newDrive) {
      if (await _googleDriveSync(
        false,
      ...
      driveSuccess = await _googleDriveSync(false,
          onFull: onFull, context: context, nav: nav);
    }
    if (driveSuccess) {
  ...
  Future<bool> _googleDriveSync(bool initial,
      {VoidCallback? onFull,
      String scope = drive.DriveApi.driveFileScope,
      BuildContext? context,
      NavigatorState? nav}) async {
    syncing = true;
    ...
    if (driver != null && driver!.scope != scope) {
      ...
      var okay = await driver!.setWD(scope == drive.DriveApi.driveAppdataScope
          ? "SWChars"
          : "SWAssistant");
      if (!okay) {
    ...
    if (driver == null && onFull == null) {
      throw ("When Driver is created, must provide onFull");
    }
    driver ??= Driver(scope, onError: (e, s) async {
    ...
    okay = await driver!.setWD(
        scope == drive.DriveApi.driveAppdataScope ? "SWChars" : "SWAssistant");
    if (!okay) {
      if (context != null) nav?.pop();
      syncing = false;
      return false;
    ...
    for (var fil in await driver!.listFiles("") ?? <drive.File>[]) {
      ...
      ed.cloudLoad(this, fil.id!).then((value) async {
        ...
        if (ed.lastMod == null || matchEd.lastMod == null) {
          ...
          if (fil.modifiedTime == null ||
              local.lastModifiedSync().isBefore(fil.modifiedTime!)) {
            loadWaiting++;
  ...
  void remove(Editable ed, [BuildContext? context]) {
    if (context != null &&
        ed.route != null &&
        observatory.containsRoute(route: ed.route) != null) {
  ...
  List<Editable> getList({Type? type, String? search, String? category}) {
    ...
    if (type == null) {
      searchList = <Editable>[..._char, ..._min, ..._veh];
    } else {
      switch (type) {
    ...
    if (search != null) {
      searchList = searchList
          .where(
            (ed) => ed.name.toLowerCase().contains(search.toLowerCase()),
          )
          .toList();
    }
    if (category != null) {
  ...
  Editable? getEditable(String uid) {
    try {
      return <Editable>[..._char, ..._min, ..._veh]
    ...
    FilePicker.platform
      ...
      if (value == null || value.files.isEmpty) {
        ...
        message.showSnackBar(
          SnackBar(
            content: Text(locale.importNone),
          ),
        );
      } else {
        message.clearSnackBars();
        message.showSnackBar(
          SnackBar(
            content: Text(
              locale.importSuccess(value.files.length),
            ),
          ),
        );
      }
