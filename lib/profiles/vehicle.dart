...
class Vehicle extends Editable {
  ...
  Vehicle(
      {super.name = "New Vehicle", super.saveOnCreation, required super.app});
  ...
  void loadJson(Map<String, dynamic> json, bool subtractMode) {
    ...
    if (json["hull trauma current"] != null) {
      if (subtractMode) {
        hullTraumaDmg =
            hullTraumaThresh - (json["hull trauma current"] ?? 0) as int;
      } else {
    ...
    if (json["system stress current"] != null) {
      if (subtractMode) {
        sysStressDmg =
            sysStressThresh - (json["system stress current"] ?? 0) as int;
      } else {
  ...
  Map<String, dynamic> toJson() => {
        ...
        "model": model,
          ...
          if (key == "defense" &&
              (value as List<int>).every((element) => element == 0)) {
            return true;
          }
  ...
  List<EditContent> cardContents(BuildContext context) => [
        ...
        EditContent(content: VehicleDefense(key: defKey), contentKey: defKey),
        EditContent(
