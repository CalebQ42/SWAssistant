...
class Minion extends Editable with Creature {
  ...
  Map<String, dynamic> toJson() => super.toJson()
    ..addAll({
      ...
      "Saved": {
        "Inventory":
            List.generate(savedInv.length, (index) => savedInv[index].toJson()),
        "Weapons": List.generate(
            savedWeapons.length, (index) => savedWeapons[index].toJson())
      }
    })
  ...
  List<EditableContent> cardContents(BuildContext context) {
    ...
    return [
      ...
      EditableContent(
          builder: (b, refresh, state) => Skills(editing: b, refresh: refresh),
      ...
      EditableContent(
          ...
          additionalButtons: () => [
                Tooltip(
                    message: savedWeapons.isEmpty
                        ? AppLocalizations.of(context)!.saveWeaponsFirst
                        : AppLocalizations.of(context)!.restoreWeapons,
                    child: IconButton(
                        iconSize: 20.0,
                        splashRadius: 20,
                        padding: const EdgeInsets.all(5.0),
                        constraints:
                            BoxConstraints.tight(const Size.square(30.0)),
                        icon: const Icon(Icons.restore),
                        onPressed: savedWeapons.isEmpty
                            ? null
                            : () {
                ...
                Tooltip(
                    message: savedWeapons.isEmpty
                        ? AppLocalizations.of(context)!.saveWeapons
                        : AppLocalizations.of(context)!.overwriteWeapons,
                    child: IconButton(
                        iconSize: 20.0,
                        splashRadius: 20,
                        padding: const EdgeInsets.all(5.0),
                        constraints:
                            BoxConstraints.tight(const Size.square(30.0)),
                        icon: Icon(savedWeapons.isEmpty
                            ? Icons.save_outlined
                            : Icons.save),
                        onPressed: () {
                          var reload =
                          ...
                          ScaffoldMessenger.of(context).showSnackBar(SnackBar(
                            content: Text(
                                AppLocalizations.of(context)!.weaponsSaved),
                          ));
                        })),
              ]),
      EditableContent(
        builder: (b, refresh, state) => Talents(editing: b, refresh: refresh),
      ...
      EditableContent(
        ...
        additionalButtons: () => [
          Tooltip(
              message: savedInv.isEmpty
                  ? AppLocalizations.of(context)!.saveInventoryFirst
                  : AppLocalizations.of(context)!.restoreInventory,
              child: IconButton(
                  iconSize: 20.0,
                  splashRadius: 20,
                  padding: const EdgeInsets.all(5.0),
                  constraints: BoxConstraints.tight(const Size.square(30.0)),
                  icon: Icon(savedInv.isEmpty
                      ? Icons.restore_outlined
                      : Icons.restore),
                  onPressed: savedInv.isEmpty
                      ? null
                      : () {
          ...
          Tooltip(
              message: savedInv.isEmpty
                  ...
                  : AppLocalizations.of(context)!.overwriteInventory,
              child: IconButton(
                  iconSize: 20.0,
                  splashRadius: 20,
                  padding: const EdgeInsets.all(5.0),
                  constraints: BoxConstraints.tight(const Size.square(30.0)),
                  icon:
                      Icon(savedInv.isEmpty ? Icons.save_outlined : Icons.save),
                  onPressed: () {
                    ...
                    ScaffoldMessenger.of(context).showSnackBar(SnackBar(
                      content:
                          Text(AppLocalizations.of(context)!.inventorySaved),
                    ));
                  }))
        ],
      ),
      ...
      EditableContent(
          builder: (b, refresh, state) => Description(editing: b, state: state),
