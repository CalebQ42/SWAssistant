  ...
  Map<String, dynamic> toJson() => {
        ...
        "Saved": {
          "Inventory": List.generate(
              savedInv.length, (index) => savedInv[index].toJson()),
        ...
        extraButtons: (context, b) => [
          Tooltip(
            message: savedWeapons.isEmpty
                ? app.locale.saveWeaponsFirst
                : app.locale.restoreWeapons,
            child: MiniIconButton(
              ...
              onPressed: savedWeapons.isEmpty
                  ...
                  : () {
                      ...
                      weaponKey.currentState
                          ?.update(() => weapons = List.of(savedWeapons));
                      ...
                      ScaffoldMessenger.of(context).showSnackBar(
                        SnackBar(
                          ...
                          action: SnackBarAction(
                            ...
                            onPressed: () => weaponKey.currentState
                                ?.update(() => weapons = tmp),
                          ),
                        ),
                      );
          ...
          Tooltip(
            message: savedWeapons.isEmpty
                ? app.locale.saveWeapons
                : app.locale.overwriteWeapons,
            child: MiniIconButton(
              icon:
                  Icon(savedWeapons.isEmpty ? Icons.save_outlined : Icons.save),
              onPressed: () {
                var reload = (savedWeapons.isEmpty && weapons.isNotEmpty) ||
                    (savedWeapons.isNotEmpty && weapons.isEmpty);
                if (reload) {
                  weaponKey.currentState
                      ?.update(() => savedWeapons = List.of(weapons));
                }
                ScaffoldMessenger.of(context).clearSnackBars();
                ScaffoldMessenger.of(context).showSnackBar(
                  SnackBar(
        ...
        extraButtons: (context, b) => [
          Tooltip(
            message: savedInv.isEmpty
                ? app.locale.saveInventoryFirst
                : app.locale.restoreInventory,
            child: MiniIconButton(
              icon: Icon(
                  savedInv.isEmpty ? Icons.restore_outlined : Icons.restore),
              onPressed: savedInv.isEmpty
                  ? null
                  : () {
                      var tmp = List.of(inventory);
                      invKey.currentState
                          ?.update(() => inventory = List.of(savedInv));
                      ScaffoldMessenger.of(context).clearSnackBars();
                      ScaffoldMessenger.of(context).showSnackBar(
                        SnackBar(
                          ...
                          action: SnackBarAction(
                            label: app.locale.undo,
                            onPressed: () => invKey.currentState
                                ?.update(() => inventory = tmp),
                          ),
                        ),
                      );
                    },
          ...
          Tooltip(
            message: savedInv.isEmpty
                ? app.locale.saveInventory
                : app.locale.overwriteInventory,
            child: MiniIconButton(
              ...
              onPressed: () {
                ...
                if (refresh) {
                  invKey.currentState
                      ?.update(() => savedInv = List.of(inventory));
                }
                ScaffoldMessenger.of(context).clearSnackBars();
                ScaffoldMessenger.of(context).showSnackBar(
                  SnackBar(
