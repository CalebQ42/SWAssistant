        ...
        child: Scaffold(
          appBar: PreferredSize(
            ...
            preferredSize: Size.fromHeight(
                Theme.of(context).appBarTheme.toolbarHeight ?? 55),
          ),
          // drawer: SWDrawer(),
          body: Row(
            ...
            children: [
              ConstrainedBox(
                  ...
                  child: EditableList(-1, key: message.listKey, onTap: (ed) {
              ...
              Expanded(
                  key: ValueKey(remain), child: _GMModeEditor(message, remain))
            ],
          ),
        ));
  }
...
class _BarState extends State<_GMModeBar> {
  ...
  void initState() {
    super.initState();
    widget.message.onChange.add((ed) => setState(() => editable = ed));
  }

  @override
  Widget build(BuildContext context) => SWAppBar(context,
      ...
      additionalPopupActions: [
        ...
        if (editable != null)
          PopupMenuItem(
              child: Text(AppLocalizations.of(context)!.clone), value: "clone"),
      ],
      popupFunctions: editable != null
          ? {
              "disableForce": () {
                setState(() => (editable as Character).disableForce =
                    !(editable as Character).disableForce);
                widget.message.editingState();
              },
              "disableDuty": () {
                setState(() => (editable as Character).disableDuty =
                    !(editable as Character).disableDuty);
                widget.message.editingState();
              },
              "disableObligation": () {
                setState(() => (editable as Character).disableObligation =
                    !(editable as Character).disableObligation);
                widget.message.editingState();
              },
              "disableMorality": () {
                setState(() => (editable as Character).disableMorality =
                    !(editable as Character).disableMorality);
                widget.message.editingState();
              },
              "clone": () => Bottom(child: (context) {
                    var nameController = TextEditingController(
                        text: AppLocalizations.of(context)!
                            .copyOf(editable!.name));
                    return Wrap(
                      children: [
                        Container(height: 10),
                        TextField(
                          ...
                          children: [
                            TextButton(
                              ...
                              onPressed: () {
                                ...
                                widget.message.listKey.currentState
                                    ?.setState(() {});
                              },
                            ),
                            TextButton(
...
class _GMModeState extends State<_GMModeEditor> {
  ...
  void initState() {
    super.initState();
    widget.message.onChange.add((ed) => setState(() => curEdit = ed));
