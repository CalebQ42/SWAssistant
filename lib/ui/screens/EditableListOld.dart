...
class _EditableListState extends State{
  ...
  final GMModeMessager? message;
  final GlobalKey<AnimatedListState> editListKey = GlobalKey();
  // bool open = false;
      ...
      return Scaffold(
        ...
        appBar: SWAppBar(
          ...
          title: AnimatedSwitcher(
            ...
            //TODO: search box that isn't terrible.
            child: Row(
              children: [
                Text(
                  (){
        ...
        floatingActionButton: FloatingActionButton(
          ...
          onPressed: (){
            ...
            switch (type){
              case 0:
                temp = new Character(saveOnCreation: true, app: SW.of(context));
                break;
              case 1:
                temp = new Minion(saveOnCreation: true, app: SW.of(context));
                break;
              default:
                temp = new Vehicle(saveOnCreation: true, app: SW.of(context));
                break;
            }
            SW.of(context).add(temp);
            list.add(temp);
            editListKey.currentState?.insertItem(list.length-1);
          }
        ),
      );
  }
...
class EditableCard extends StatelessWidget{
  ...
  EditableCard({this.key, required this.editable, required this.refreshCallback, required this.upperContext, this.onTap, required this.list});
  @override
  Widget build(BuildContext context) =>
    Dismissible(
      key: UniqueKey(),
