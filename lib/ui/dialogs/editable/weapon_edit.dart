...
class WeaponEditDialog {
  ...
  WeaponEditDialog(
    ...
    damageController = TextEditingController(
        text: weapon.damage != -1 ? weapon.damage.toString() : "")
      ..addListener(() {
        weapon.damage = int.tryParse(damageController.text) ?? -1;
        bot.updateButtons();
      });
    criticalController = TextEditingController(
        text: weapon.critical != -1 ? weapon.critical.toString() : "")
      ..addListener(() {
    ...
    encumbranceController = TextEditingController(
        text: weapon.encumbrance.toString())
      ..addListener(() =>
          weapon.encumbrance = int.tryParse(encumbranceController.text) ?? 0);
    arcController = TextEditingController(text: weapon.firingArc)
      ..addListener(() => weapon.firingArc = arcController.text);
    bot = Bottom(
        buttons: (context) => [
              TextButton(
                  ...
                  child: Padding(
                      ...
                      child: Row(
                        children: [
                          ...
                          Text(widget.weapon.characteristics[i].advantage
                              .toString())
                        ],
                      )))),
          IconButton(
        ...
        ..add(
          TextButton.icon(
            ...
            onPressed: () => WeaponCharacteristicDialog(
                    onClose: (wc) =>
                        setState(() => widget.weapon.characteristics.add(wc)))
                .show(context),
          ),
        ));
}
      ...
      InputDecorator(
          ...
          child: DropdownButton<int>(
            ...
            items: List.generate(
                weaponSkills.length,
                (i) =>
                    DropdownMenuItem(child: Text(weaponSkills[i]), value: i)),
            value: widget.weapon.skill == -1 ? null : widget.weapon.skill,
            onChanged: (value) => setState(() {
              widget.weapon.skill = value ?? -1;
              widget.weapon.skillBase =
                  Skill.skillsList(context)[weaponSkills[widget.weapon.skill]]!;
          ...
          child: DropdownButton<int>(
            ...
            items: List.generate(
                ...
                (i) => DropdownMenuItem(
                    ...
                    value: i)),
            value:
                widget.weapon.skillBase == -1 ? null : widget.weapon.skillBase,
            onChanged: (value) => setState(() {
