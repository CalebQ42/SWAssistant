...
class WeaponCharacteristicDialog {
  ...
  WeaponCharacteristicDialog(
    ...
    valueController =
        TextEditingController(text: wc.value != -1 ? wc.value.toString() : "")
          ..addListener(() {
            wc.value = int.tryParse(valueController.text) ?? -1;
            bot.updateButtons();
          });
    advantageController = TextEditingController(
        text: wc.advantage != -1 ? wc.advantage.toString() : "")
      ..addListener(() {
      ...
      child: (context) => Wrap(
        children: [
          ...
          TextField(
            ...
            decoration:
                InputDecoration(labelText: AppLocalizations.of(context)!.rank),
