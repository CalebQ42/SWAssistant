...
class EditingText extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (editing) {
      text = Padding(
          ...
          child: TextField(
            ...
            decoration:
                InputDecoration(isCollapsed: collapsed, labelText: title),
          ));
    } else {
      text = Padding(
              ...
              Text(
                ...
                style: titleStyle ??
                    Theme.of(context)
                        .textTheme
                        .bodyText1
                        ?.apply(fontSizeDelta: .75, fontWeightDelta: 2),
                textAlign: textAlign,
              ),
      ...
      transitionBuilder: (wid, anim) {
        ...
        return ClipRect(
            child: SlideTransition(
                position: slide.animate(anim), child: Center(child: wid)));
      },
      layoutBuilder: (child, oldStack) {
        List<Widget> newStack = [];
        for (var chil in oldStack) {
          newStack.add(SizedOverflowBox(size: Size.zero, child: chil));
        }
    ...
    return AnimatedSize(
        duration: const Duration(milliseconds: 300), child: switcher);
  }
}

class EditableContent extends StatefulWidget {
  final Widget Function(
      bool editing, Function() refresh, EditableContentState state)? builder;
...
class EditableContentState extends State<EditableContent> {
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      ...
      children: <Widget>[
        widget.builder != null
            ? widget.builder!(editing, () => setState(() {}), this)
            : widget.stateful!,
        Row(mainAxisAlignment: MainAxisAlignment.end, children: [
          if (widget.editingButtons != null)
            AnimatedSwitcher(
          ...
          if (widget.editButton || addButtons.isNotEmpty)
            ButtonBar(
