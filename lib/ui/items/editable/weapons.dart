...
class WeaponsState extends State<Weapons> with StatefulCard {
  ...
  Widget build(BuildContext context) {
    ...
    var weaponsList = List.generate(
      editable.weapons.length,
      (i) => InkResponse(
        ...
        onTap: () {
          if (editable.weapons[i].itemState == 4) {
          ...
          } else if ((editable.weapons[i].limitedAmmo &&
                  editable.weapons[i].ammo <= 0) ||
              !editable.weapons[i].loaded) {
            ScaffoldMessenger.of(context).clearSnackBars();
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(
            ...
            var pack = WeaponPack(editable.weapons[i],
                (editable is Creature) ? editable.charVals[0] : 0);
            if (editable is Creature) {
              var skill = editable.skills.firstWhere(
                (element) =>
                    element.name ==
                    Weapon.weaponSkills(context)[editable.weapons[i].skill!],
                orElse: () => Skill(),
              );
              if (skill.name != "" &&
                  skill.base == editable.weapons[i].skillBase) {
                var hold = SWDiceHolder(
                  ability: ((skill.value ?? 0) -
                          (editable.weapons[i].skillBase != null
                              ? editable
                                  .charVals[editable.weapons[i].skillBase!]
                              : 0))
                      .abs(),
                  proficiency: min(
                      (skill.value ?? 0),
                      (editable.weapons[i].skillBase != null
                          ? editable.charVals[editable.weapons[i].skillBase!]
                          : 0)),
                  weaponPack: pack,
                );
                var accInd = editable.weapons[i].characteristics.indexWhere(
                    (element) =>
                        element.name == app.locale.characteristicAccurate);
                var inaccInd = editable.weapons[i].characteristics.indexWhere(
                    (element) =>
                        element.name == app.locale.characteristicInaccurate);
                //TODO: all passive characteristics
                if (accInd != -1) {
                  hold.boost =
                      editable.weapons[i].characteristics[accInd].value ?? 1;
                }
                if (inaccInd != -1) {
                  hold.setback =
                      editable.weapons[i].characteristics[inaccInd].value ?? 1;
                }
                hold.weaponPack = pack;
                hold.showDialog(context);
              } else {
                SWDiceHolder(
                  ability: (editable.weapons[i].skillBase != null
                      ? editable.charVals[editable.weapons[i].skillBase!]
                      : 0),
                  weaponPack: pack,
                ).showDialog(context);
                    ...
                    child: (context) {
                      ...
                      return Wrap(
                        ...
                        children: [
                          ...
                          if (editable.weapons[i].limitedAmmo)
                            Container(height: 5),
                          if (editable.weapons[i].limitedAmmo)
                            Center(
                              child: Text(
                          ...
                          if (editable.weapons[i].characteristics.isNotEmpty)
                            Container(height: 10),
                          if (editable.weapons[i].characteristics.isNotEmpty)
                            Center(
                              child: Text(app.locale.characteristicPlural,
                                  style:
                          ...
                          if (editable.weapons[i].characteristics.isNotEmpty)
                            ...List.generate(
                              ...
                              (ch) {
                                ...
                                var char = editable.weapons[i]
                                    .characteristics[(ch / 2).floor()];
                                String out = char.name;
                                if (char.value != 1 && char.value != null) {
                                  out += " ${char.value}";
                                }
                                if (char.advantage != null) {
                                  out +=
                                      " ${app.locale.advNeeded}: ${char.advantage}";
                                }
                                return Center(
                                  child: Text(
                                    out,
                  ...
                  : OverflowBar(
                      children: [
                        ...
                        IconButton(
                          ...
                          onPressed: () => WeaponEditDialog(
                            editable: editable,
                            onClose: (weapon) {
                              if (editable.weapons[i].encumbrance !=
                                  weapon.encumbrance) {
