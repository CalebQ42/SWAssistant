...
class VehicleDamageState extends State<VehicleDamage> with StatefulCard {
  ...
  Widget build(BuildContext context) {
    ...
    hullTraumaThreshController ??=
        TextEditingController(text: vehicle.hullTraumaThresh.toString())
          ..addListener(() => vehicle.hullTraumaThresh =
              int.tryParse(hullTraumaThreshController!.text) ?? 0);
    sysStressThreshController ??=
        TextEditingController(text: vehicle.sysStressThresh.toString())
          ..addListener(() => vehicle.sysStressThresh =
              int.tryParse(sysStressThreshController!.text) ?? 0);
                  ...
                  child: !edit
                      ? Column(
                          children: [
                            ...
                            Center(
                              child: Text(
                                app.locale.max(vehicle.hullTraumaThresh),
                              ),
                            )
                          ],
                        )
                      : TextField(
                          ...
                          decoration:
                              InputDecoration(labelText: app.locale.maxTrauma),
                          textAlign: TextAlign.center,
                        ),
                  ...
                  child: !edit
                      ? Column(
                          children: [
                            ...
                            Center(
                              child: Text(
                                app.locale.max(vehicle.sysStressThresh),
                              ),
                            )
                          ],
                        )
                      : TextField(
                          ...
                          keyboardType: TextInputType.number,
                          decoration:
                              InputDecoration(labelText: app.locale.maxStress),
                          textAlign: TextAlign.center,
                        ),
                  transitionBuilder: (child, anim) {
                    Tween<Offset> offset;
